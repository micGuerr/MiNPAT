...
%% Step 2.1     Run topup
config.dwi.topup.outpath = fullfile(config.dwi.path, 'topup', 'topup');
...
[config.dwi.bet.mask, ...
    config.dwi.bet.outc] = runBET(config.dwi.topup.outpath, config.dwi.bet.f);

%% Step 2.3     Run EDDY

config.dwi.eddy.outpath = fullfile(config.dwi.path, 'eddy', 'eddy');

[config.dwi.eddy.outp, ...
    config.dwi.eddy.outc] = runEddy(config.dwi.input, ...  
                                config.dwi.topup.outpath, ...
                                    config.dwi.bet.mask, ...
                                        config.dwi.eddy.outpath, ...
                                            config.parall);
